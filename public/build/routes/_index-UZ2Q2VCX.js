...
function buildTransform(transform25, { enableHardwareAcceleration = true, allowTransformNone = true }, transformIsDefault, transformTemplate) {
  let transformString = "";
  for (let i = 0; i < numTransforms; i++) {
    const key = transformPropOrder[i];
    if (transform25[key] !== void 0) {
      const transformName = translateAlias[key] || key;
      transformString += `${transformName}(${transform25[key]}) `;
    }
  }
  if (enableHardwareAcceleration && !transform25.z) {
    transformString += "translateZ(0)";
  }
  transformString = transformString.trim();
  if (transformTemplate) {
    transformString = transformTemplate(transform25, transformIsDefault ? "" : transformString);
...
function buildHTMLStyles(state, latestValues, options, transformTemplate) {
  const { style, vars, transform: transform25, transformOrigin } = state;
...
function transformBox(box, transform25) {
  transformAxis(box.x, transform25, xKeys);
  transformAxis(box.y, transform25, yKeys);
}
...
function buildProjectionTransform(delta, treeScale, latestTransform) {
  ...
  if (xTranslate || yTranslate) {
    transform25 = `translate3d(${xTranslate}px, ${yTranslate}px, 0) `;
  }
  if (treeScale.x !== 1 || treeScale.y !== 1) {
    transform25 += `scale(${1 / treeScale.x}, ${1 / treeScale.y}) `;
  }
  if (latestTransform) {
    const { rotate, rotateX, rotateY } = latestTransform;
    if (rotate)
      transform25 += `rotate(${rotate}deg) `;
    if (rotateX)
      transform25 += `rotateX(${rotateX}deg) `;
    if (rotateY)
      transform25 += `rotateY(${rotateY}deg) `;
  }
  const elementScaleX = delta.x.scale * treeScale.x;
  const elementScaleY = delta.y.scale * treeScale.y;
  if (elementScaleX !== 1 || elementScaleY !== 1) {
    transform25 += `scale(${elementScaleX}, ${elementScaleY})`;
  }
  return transform25 || "none";
}
...
var getTranslateFromMatrix = (pos2, pos3) => (_bbox, { transform: transform25 }) => {
  if (transform25 === "none" || !transform25)
    return 0;
  const matrix3d = transform25.match(/^matrix3d\((.+)\)$/);
  if (matrix3d) {
    return getPosFromMatrix(matrix3d[1], pos3);
  } else {
    const matrix = transform25.match(/^matrix\((.+)\)$/);
    if (matrix) {
...
// node_modules/@unpic/react/dist/chunk-W7EQS7E7.mjs
...
var getSrcSet = (options) => {
  ...
  return getSrcSetEntries({ ...options, transformer }).map((transform25) => {
    const url = transformer(transform25);
    return `${url?.toString()} ${transform25.width}w`;
  }).join(",\n");
};
